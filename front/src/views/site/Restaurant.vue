<template>
  ...
  <main class="w-full min-h-[calc(100vh-75px)] bg-white md:mt-[75px] md:max-w-[1200px] mx-auto">
    ...
    <div class="px-5 lg:grid grid-cols-3 gap-4 gap-y-0.5 mb-[91px]" v-if="dishes.length > 0">
      <DishCard v-for="dish in dishes" :key="dish.id" @click="selectedDish = dish" :dish="dish" />
    </div>
  </main>
  <DishModal v-if="selectedDish !== null && restaurant !== null" :dish="selectedDish" :restaurant="restaurant"
...
if (route.query.dish) {
  fetchDishes();
  const { dish, ...restQuery } = route.query;
