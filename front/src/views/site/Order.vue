...
import OrderChatClient from "@/components/site/OrderChatClient.vue";
    ...
    channel.bind(`order-status-updated_${orderId}`, (data) => {
        ...
        if (data.orderId == orderId) {
            ...
            infoFirst.value = {
                ...
                localDateTime: data.time,
            };
        }
    });

    const chatChannel = pusherService.subscribe(`chat_${orderId}`);
    chatChannel.bind("client-delivery-chat", (data) => {
...
onMounted(async () => {
    ...
    if (orderData) {
        ...
        orderInfo.value = orderData.orderInfo;
        infoFirst.value = orderInfo.value.pop();
    }
...
onUnmounted(() => {
    ...
    pusherService.unsubscribe(`chat_${orderId}`);
...
<template>
    <div class="w-full flex flex-col items-center">
        <div class="main"><!-- Cabeçalho -->
            ...
            <!-- Status do Pedido --><div class="flex flex-col items-start justify-center px-5 gap-3">
            ...
            <div class="flex flex-col items-start justify-center px-5 gap-3 py-4">
                <button
                    class="flex flex-row items-center w-full justify-between gap-4 md:pb-1"
                ...
                <transition
                    ...
                    >
                        <div class="flex flex-row justify-between" v-for="info in orderInfo" :key="info.localDateTime">
                            <div class="flex flex-row items-center w-full justify-between gap-4 font-semibold">
                                ...
                                <span class="text-tertiary-light text-sm pr-1">{{
                                        new Date(info.localDateTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
                                    }}</span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="h-[450px] w-full">
                <OrderChatClient :delivery-chat="deliveryChat" :customer-order-id="orderId" :chat="chat" />
            </div>
            <!-- Resumo do Pedido -->
            <div class="px-3 flex flex-col gap-y-0 mt-6  md:max-h-[calc(100%-15px)]"
                ...
                <div class="order-items border-s border-e px-3">
                    <div class="product" v-for="item in orderItems" :key="item.id">
...
.dropdown {
    @apply absolute top-0 right-0 flex flex-col-reverse gap-4 mt-60 px-5 w-full z-50 bg-white border-b pb-4 shadow-xl;
