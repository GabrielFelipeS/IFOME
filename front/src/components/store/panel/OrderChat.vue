    ...
    <div class="w-[95%] h-[95%] max-h-[60vh] m-auto border rounded-lg flex flex-col">
        ...
        <div ref="messageContainer" class="flex-grow overflow-y-auto p-4 space-y-3">
            <div v-for="(message, index) in currentChat" :key="index" class="w-full flex" :class="{
                'justify-end': message.senderType === 'RESTAURANT',
                'justify-start': message.senderType === (activeChat === 'client-restaurant' ? 'CLIENT' : 'DELIVERY'),
            }">
                <div :class="{
                    'bg-blue-100': message.senderType === 'RESTAURANT',
                    'bg-gray-100': message.senderType === 'DELIVERY' || message.senderType === 'CLIENT',
...
const currentStatus = computed(() => orderStatusStore.getOrderStatus(props.customerOrderId.orderId) || props.customerOrderId.orderInfo[props.customerOrderId.orderInfo.length - 1].orderStatus);
    ...
    try {
        const endpoint =
            activeChat.value === "client-restaurant"
                ...
                : `chat/restaurant/delivery/${props.customerOrderId.orderId}`;
...
onMounted(() => {
    try {
        scrollToBottom();
        const channel = pusher.subscribe(`chat_${props.customerOrderId.orderId}`);
